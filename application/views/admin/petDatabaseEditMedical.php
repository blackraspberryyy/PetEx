...
<div class ="side-nav-offset">
    <div class ="container ">
        <form action = "<?= $this->config->base_url() ?>admin/petDatabaseEditMedical_exec/<?= $records->medicalRecord_id?>/<?= $records->pet_id?>" enctype="multipart/form-data" method = "POST" runat="server">
            <div class = "card row">
                <nav class = "green darken-3">
                    <div class="col s12">
                        <a href="<?= $this->config->base_url() ?>admin/petDatabaseMedicalRecords/<?= $records->pet_id?>" class="breadcrumb">Medical Records</a>
                        <a href="<?= $wholeUrl ?>" class="breadcrumb">Edit Medical Record</a>
                    </div>
                </nav>
                        ...
                        <div class="card grey lighten-4">
                            <div class="card-content row">
                                <div class="input-field col s12 <?php if (!empty(form_error("weight"))): ?>error-theme<?php else: ?>green-theme<?php endif; ?>">
                                    <input id="weight" type="text" class="" name = "weight"  autofocus="" value="<?= set_value('weight',$records->medicalRecord_weight) ?>">
                                    <label for="weight">Weight</label>
                                </div>
                                <div class = "col s12">
                                    <div class="input-field <?php if (!empty(form_error("diagnosis"))): ?>error-theme<?php else: ?>green-theme<?php endif; ?>">
                                        <textarea id="textarea" class="materialize-textarea" name = "diagnosis"><?= set_value('diagnosis',$records->medicalRecord_diagnosis) ?></textarea>
                                ...
                                <div class = "col s12">
                                    <div class="input-field <?php if (!empty(form_error("treatment"))): ?>error-theme<?php else: ?>green-theme<?php endif; ?>">
                                        <textarea id="textarea" class="materialize-textarea" name = "treatment"><?= set_value('treatment',$records->medicalRecord_treatment) ?></textarea>
